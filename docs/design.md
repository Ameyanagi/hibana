...
This trust applies to stored invariants, not new resource requirements: exact
matching can raise when new input dimensions overflow or exceed a configured
DP workspace budget. Each validated type exposes one explicit raising
`validate()` checkpoint for
